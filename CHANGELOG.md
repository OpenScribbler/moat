...
- **`reference/moat-npm-publisher.yml` reference Publisher workflow added** — a canonical end-to-end GitHub Actions workflow that realizes the six-step npm Publisher sequence (`npm pack` v1 → compute canonical Content Directory hash → sign canonical attestation payload with Sigstore keyless and capture Rekor log index → write `moat.publisherSigning` back into `package.json` → `npm pack` v2 → `npm publish`). The two-pack pattern demonstrates that the default Content Directory rule (root `package.json` excluded from the hash domain) makes Publisher signing identity disclosable inside `package.json` without invalidating the signature. Cited from `specs/npm-distribution.md` `## Reference Implementations`. Concurrent edits: `moat-spec.md` Sub-specs line gains `specs/npm-distribution.md`; `.claude/rules/changelog.md` example path swapped from `specs/publisher-action.md` to `specs/github/publisher-action.md` (post-reorg cleanup).
- **specs/npm-distribution.md** — new sub-spec defining the npm transport binding for MOAT. Specifies the Content Hash input domain (the unpacked tarball's Content Directory, with `package.json` excluded by default), the `package.json` `moat` block schema (top-level `publisherSigning` object for Publisher identity disclosure; `attestations[]` array for Registry counter-signatures), single-path Rekor query semantics keyed on canonical Content Hash and filtered by `{issuer, subject}`, hard pre-materialization revocation anchored at the install boundary (no operator override), Registry-side backfill semantics (same `registry_signing_profile`, no second profile field), and the orthogonality between npm provenance and the MOAT Trust Tier. Companion website mirror added at `website/src/content/docs/spec/npm-distribution.md` and Specification sidebar updated.
- **`specs/npm-distribution.md` `moat.distribution_uri` field added (REQUIRED for cooperative Publishers)** — a new field in the `package.json` `moat` block disclosing the npm tarball URL (`https://registry.npmjs.org/<name>/-/<basename>-<version>.tgz`) so a Registry or Conforming Client can fetch the canonical distribution artifact without re-resolving the package against the npm Registry API. `distribution_uri` is distinct from `source_uri`: `source_uri` names the upstream git repository, `distribution_uri` names the channel-specific artifact. A Conforming Client MUST NOT infer one from the other. The reference Publisher workflow (`reference/moat-npm-publisher.yml`) constructs the URI deterministically from package name and version and writes both fields into `package.json` before the second `npm pack`.
- **`specs/npm-distribution.md` `## Conformance (normative)` section added** — a pipe table of `NPM-<SECTION>-<NN>` error codes mapping every normative MUST and MUST NOT in the sub-spec to a `specs/npm-distribution.md:<line>` citation. The table is the public surface a Conforming Client emits on a refusal so an End User (or operator reading logs) can route from refusal text back to the section of the spec that defines the rule. Codes cover the §Content Directory and Content Hash domain (`NPM-HASH-*`, `NPM-CDIR-*`), §Revocation MUSTs (`NPM-REV-*`), §Schema fields (`NPM-SCHEMA-*`), §Canonical Attestation Payload (`NPM-PAYLOAD-*`), §Dual-attestation rules (`NPM-DUAL-*`), §Publisher Verification (`NPM-PUB-*`), §Backfill (`NPM-BACKFILL-*`), and §npm Provenance (`NPM-PROV-*`).
- **`.claude/rules/spec-citations.md`** — new contributor-facing rule codifying the `<path>:<line>` citation form used by the §Conformance error-code table. The rule names the canonical example (`specs/npm-distribution.md:117`), states the convention for keeping citations current under spec edits, and points at the §Conformance table as the load-bearing user of the form.
- **`NPM-SCOPE-01-A` / `NPM-SCOPE-01-B` rows added to the §Conformance table (`specs/npm-distribution.md`)** — the rule that a Conforming Client encountering a `distribution_uri` whose host is not `registry.npmjs.org` MUST treat the item as outside this sub-spec's normative coverage is surfaced as two distinct error codes so a refusal can route to the specific failure mode: `NPM-SCOPE-01-A` for the wrongly-attested case (Conforming Client emitted a positive attestation on the basis of this sub-spec) and `NPM-SCOPE-01-B` for the wrongly-refused case (Conforming Client emitted a refusal on the basis of this sub-spec). The MUST has been lifted out of the former `### Out of Scope` "Private-registry backfill" paragraph (which was the wrong home — `### Out of Scope` is non-normative by convention) into a normative §Scope paragraph plus two co-adjacent MUST NOT sentences; the rationale paragraph in `### Out of Scope` has been de-MUSTed accordingly, with a back-reference to the new normative paragraphs. The single-letter split-suffix (`-A`/`-B`) is a permitted extension to the `NPM-<SECTION>-<NN>` code-format pattern, allowing one MUST/MUST NOT to be split into co-adjacent failure-mode variants without colliding with the linear `-NN` sequence. Sub-spec `specs/npm-distribution.md` version bumped to 0.3.0 (Draft). ADR-0015 gains an addendum naming the split as the first use of the suffix form. Normative reinforcement — the obligation is unchanged in scope, but is now stated under a normative heading and surfaced as two citable refusal codes.
- **Code-stability lock file (`specs/conformance/npm-distribution/error-codes.lock`)** — names every `NPM-<SECTION>-<NN>(-<X>)?` error code the sub-spec has committed to (41 codes at this revision). The lock makes the ADR-0015 stability contract programmatic: a new slice-8 conformance assertion (A10) verifies on every CI run that every locked code still appears in the §Conformance table, either as a live row or as `Reserved (was: <description>)` per ADR-0015. The table is free to grow beyond the lock — additions do not require a lock entry — but renames and removals fail the build. New entries are appended to the lock file in the same PR that cuts the spec to a release tag. ADR-0015 gains an addendum naming the lock file as the programmatic enforcement of the stability contract previously asserted only on the spec surface.
- **Slice-8 body-prose citation lint (A9)** — `specs/conformance/npm-distribution/slice-8-error-codes.sh` grows a new conformance assertion that verifies every `<path>:<line>` citation in the spec body outside `## Conformance (normative)` points at a line carrying `MUST`, `MUST NOT`, or `SHOULD`. This closes the gap left by A6/A7, which only validate citations inside the §Conformance table; cross-spec references in the spec body (e.g., the `moat-spec.md:636` and `moat-spec.md:807` anchors in §Revocation and §Backfill) are now monitored on every CI run.

### Changed

- **Revocation override scope (`specs/npm-distribution.md`)** — the no-operator-override MUST NOT is scoped to Registry-source revocations (those recorded in `.moat/npm-lockfile.json` `revoked_hashes`). Publisher-source revocations defer to `moat-spec.md §Revocation`'s Publisher row at `moat-spec.md:636`, which allows use with explicit End User confirmation. Normative clarification — resolves an unintended cross-spec contradiction between the npm sub-spec and core.
- **Citation uniqueness for the four-code REV and SCHEMA hotspots (`specs/npm-distribution.md`)** — the Revocation paragraph that previously packed four MUSTs into a single line is split into four normative sentences (pre-materialization lockfile consultation, refusal on revoked Content Hash, revocation-reason-code surfacing, lockfile-authoritative-over-remote-source), and the `moat.distribution_uri` schema row is de-MUSTed into description-only with four MUST sentences lifted into prose paragraphs below the schema table (cooperative declaration, Registry backfill, resolution to attested tarball, source/distribution separation). The `## Conformance (normative)` table's eight affected codes (`NPM-REV-01..04`, `NPM-SCHEMA-02..05`) now each cite a distinct `specs/npm-distribution.md:<line>` anchor, restoring the one-MUST-per-code property guaranteed by the citation convention. Normative restructuring — every original MUST is preserved verbatim in scope; the change is in how many lines they occupy.
- **Citation uniqueness across the remaining multi-anchored lines (`specs/npm-distribution.md`)** — the remaining eight multi-MUST source lines are split per the same methodical pattern: the Content Directory Default paragraph (`NPM-CDIR-01..02`), the Fixed-exclusion-list paragraph (`NPM-CDIR-04..05`), the no-operator-override paragraph (`NPM-REV-06..07`), the Dual-attestation sentence (`NPM-DUAL-01..02`), the Publisher-verification Query algorithm paragraph (`NPM-PUB-01..03`), the Anti-rollback paragraph (`NPM-PUB-05..06`), the backfill `source_uri`/`distribution_uri` paragraph (`NPM-BACKFILL-02..03`), and the Uniqueness-invariant paragraph (`NPM-BACKFILL-04..05`). The `## Conformance (normative)` table is retargeted in lockstep: every `NPM-<SECTION>-<NN>` row now cites a distinct `specs/npm-distribution.md:<line>` anchor (`sort | uniq -d` on the citation column is empty). Normative restructuring — every original MUST is preserved verbatim in scope; the change is in how many lines they occupy.
- **Code stability cover added to the §Conformance intro (`specs/npm-distribution.md`)** — the `## Conformance (normative)` intro paragraph now carries three normative sentences that hold conformers to the error-code stability contract on the spec surface itself: once shipped, a code's spelling and meaning MUST NOT change; obsolete codes MUST be retained in the table marked `Reserved (was: <description>)` rather than reused for a different obligation; Conforming Clients SHOULD surface codes verbatim. ADR-0015's Consequences section gains an addendum that cites the spec intro line and names the slice-8 conformance lint that enforces the citation-anchor half of the guarantee on every CI run. Normative reinforcement — the stability guarantee was already a Decision in ADR-0015; this change places the obligation on the spec surface a Conforming Client implementer reads.
- **Citation-uniqueness lint added to the slice-8 conformance script** — `specs/conformance/npm-distribution/slice-8-error-codes.sh` grows a new `A7` check that fails the build when any two `NPM-<SECTION>-<NN>` codes in the `## Conformance (normative)` table cite the same `specs/npm-distribution.md:<line>` anchor. This pairs with the pre-existing `A6` (every citation points at a line carrying `MUST`/`MUST NOT`) to enforce the one-MUST-per-code property on every CI run. The previous `A7` (website mirror H1 parity) is renumbered to `A8`; the script header lists both. The slice-8 script's prose self-description is brought into agreement with its filename — the lingering `slice-7` tokens at the header comment, the pre-flight `FAIL` line, and the final `PASS`/`FAIL` lines are now `slice-8`.
- **Slice-8 A6 and A7 broadened to accept cross-spec citations** — both assertions now match `<path>:<line>` anchors of the form `(specs/**/*.md|moat-spec.md):<line>` rather than the previous `specs/npm-distribution.md:<line>`-only form, and A6 reads the cited line from the cited path (not from `specs/npm-distribution.md` unconditionally). The `## Conformance (normative)` table is permitted to cite normative MUSTs in `moat-spec.md` or in sibling sub-specs when an obligation originates there, which the table did not previously support. Editorial — no normative change to the spec body.
- **`specs/npm-distribution.md` npm Provenance section: two-column state table plus normative prose** — the `## npm Provenance` section presents the four observable states (both present, MOAT-only, provenance-only, neither) as a two-column table (`State` × `Trust Tier impact`) so the table carries description-only content and every normative MUST / MUST NOT lands in adjacent prose paragraphs. The both-present case is normative MUST NOT: a Conforming Client MUST NOT infer one signal from the other. The provenance-only case is normative MUST: when npm provenance is present but no MOAT attestation is present, a Conforming Client MUST display the package's MOAT Trust Tier as `Unsigned`. The orthogonality rule from earlier drafts is preserved; the restructuring places the normative obligations where a `grep MUST` finds them, not buried inside table cells.
- **`specs/npm-distribution.md` Publisher signing identity disclosed via `publisherSigning` block (Draft-status breaking change)** — the Publisher's signing identity lives in a top-level `moat.publisherSigning` object with REQUIRED `issuer` and REQUIRED `subject`. The Publisher's Sigstore bundle is not embedded in `package.json`; it lives in the Rekor transparency log, and `publisherSigning` is the metadata a Conforming Client uses to locate it. `moat.attestations[]` is scoped to Registry attestations only. The `## Publisher Verification (normative)` section defines a single normative query algorithm (no discovery accelerator): the Client MUST query Rekor by canonical Content Hash, MUST filter the result set by the `{issuer, subject}` pair, and MUST refuse if zero entries match. The Client MUST also verify that the selected entry's signing-certificate identity exactly matches `{issuer, subject}` — payload match alone is insufficient. The v0.1.0 "duplicate role is malformed" rule is replaced by structural JSON schema enforcement (Publisher cardinality is now structural: `publisherSigning` is a single object). Sub-spec is Draft and has no adopters; no migration scaffolding is provided.
...
- **`specs/npm-distribution.md` revocation is hard at the materialization boundary (normative — MUST)** — a Conforming Client that detects a revoked Content Hash MUST refuse to materialize the package and MUST emit a structured log entry that records the revocation `source` in addition to the reason code (the `source` field discriminates lockfile-entry revocations from Registry-Manifest-entry revocations; the reason code remains the operator-facing classifier — `malicious`, `compromised`, `deprecated`, `policy_violation`). There is no operator override mechanism in the protocol; an implementation that exposes any per-hash, per-package, or global bypass is non-conformant. The structured log entry is what gives an operator the artifact needed to escalate to the upstream revocation source rather than locally suppressing the refusal.
- **`specs/npm-distribution.md` `moat.contentDirectory` renamed to `moat.tarballContentRoot`** — the field-name rename realizes the lexicon's **Content Directory** concept inside the npm Distribution Channel: the lexicon term remains the source of truth, and `tarballContentRoot` names the tarball-relative subdirectory that maps to it. Renamed in the field table, JSON examples, and prose throughout the sub-spec. The field is REQUIRED — a cooperative Publisher MUST declare it explicitly, and it MAY be omitted only on Registry-backfilled items (for which the Registry populates the canonical Content Hash by applying the default Content Directory rule). The Description column for `moat.tarballContentRoot` cross-references the lexicon Content Directory entry rather than redefining the concept inline. `lexicon.md`'s Content Directory entry gains a one-line realization note pointing at `tarballContentRoot`. No migration scaffolding is provided; the sub-spec is Draft with zero adopters.
...
- **Conformance scripts for `specs/npm-distribution.md` now live at `specs/conformance/npm-distribution/`** — the slice conformance scripts and inline variants are committed alongside the sub-spec they verify. The `## Reference Implementations` citation in `specs/npm-distribution.md` and the comment in `reference/moat-npm-publisher.yml` are updated to the new path. No normative change.
- **`specs/npm-distribution.md` `### Out of Scope` clause for private-registry backfill** — the §Scope section gains an Out of Scope subsection naming private-registry backfill (auth-required, access-controlled npm-protocol-compatible registries) as explicitly out of scope for the public-registry sub-spec. The clause names the three open design questions that must be answered before private-registry support can be specified: how a Registry authenticates to fetch a private tarball, how `distribution_uri` disambiguates between private-registry hosts when URLs are not globally unique, and how a Conforming Client distinguishes public-registry revocations from private-registry revocations. The corresponding `ROADMAP.md` entry is added under **Private registry authentication**.

### Fixed

- **Mechanical-drift sweep across `specs/npm-distribution.md`, `docs/adr/0013-where-the-new-distribution-uri-field-lives-in-the-package-js.md`, and this CHANGELOG** — the canonical npm Publisher sequence is six steps (the prose previously called it seven because it counted the bundle-and-Rekor capture as two separate steps); the `moat.tarballContentRoot` rename bullet now reflects the field's REQUIRED status (the bullet had stated it was OPTIONAL, which contradicted the schema row that requires cooperative Publishers to declare it explicitly); ADR-0013's Decision and Consequences paragraphs now cite `specs/npm-distribution.md:79` (the schema-table header) and `specs/npm-distribution.md:111` (the worked-example JSON open) instead of brittle line ranges (`:88–98` and `:117–122`) that drift on any spec edit above the cited block; `specs/npm-distribution.md` §Schema gains a one-paragraph editorial note above the schema table explaining the camelCase/snake_case mix (snake_case field names preserve spelling parity with the upstream `moat-spec.md` and Rekor-bundle identifiers they mirror, rather than renaming them at the sub-spec boundary); the §Backfill paragraph's cross-spec reference to `moat-spec.md` is now `moat-spec.md:807` (the per-item compound-key uniqueness MUST that also pins `source_uri` as REQUIRED on every manifest entry) instead of a brittle `lines 766–807` range; and this CHANGELOG's revocation narrative bullet now states that the structured log entry records the revocation `source` in addition to the reason code (rather than implying `source` replaces the reason code — both fields are emitted, and they answer different operator questions). Editorial sweep — no normative change; the underlying obligations are unchanged in scope.
...
- **`specs/npm-distribution.md` `publisherSigning.rekorLogIndex` field** — the OPTIONAL Rekor-log-index discovery accelerator is removed from the schema. The field carried no security value once the `{issuer, subject}` identity check became the authoritative trust anchor, and its existence invited implementations that trusted the index and skipped the identity verification. The single normative query path remains: query Rekor by canonical Content Hash, filter by `{issuer, subject}`, refuse on zero matches. A package published under an earlier Draft schema that carried `rekorLogIndex` remains parseable — the field is ignored on read. Recorded in `docs/adr/0011-*.md` (supersedes ADR-0008).
- **`specs/npm-distribution.md` `MOAT_ALLOW_REVOKED` operator override mechanism** — the per-hash environment-variable escape hatch (and its REQUIRED co-variable `MOAT_ALLOW_REVOKED_REASON`, per-entry RFC 3339 expiry encoding, and structured override-applied event) is removed from the protocol. A Conforming Client that detects a revoked Content Hash MUST refuse to materialize the package; there is no per-hash, per-package, or global bypass. Incident-response workflows route through correcting the authoritative source (lockfile or Registry Manifest) rather than overriding locally. Recorded in `docs/adr/0010-*.md` (supersedes ADR-0007).
