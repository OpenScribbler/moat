...
- **`NPM-SCOPE-01` row added to the §Conformance table (`specs/npm-distribution.md`)** — the rule that a Conforming Client encountering a `distribution_uri` whose host is not `registry.npmjs.org` MUST treat the item as outside this sub-spec's normative coverage now has its own error-code row, citing the new normative paragraph at the bottom of §Scope (above `### Out of Scope`). The MUST sentence has been lifted out of the former `### Out of Scope` "Private-registry backfill" paragraph (which was the wrong home — `### Out of Scope` is non-normative by convention) and the rationale paragraph there has been de-MUSTed accordingly, with a back-reference to the new normative paragraph. Normative reinforcement — the obligation is unchanged in scope, but is now stated under a normative heading and surfaced as a citable refusal code.
