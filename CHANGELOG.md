...
- **Conformance scripts for `specs/npm-distribution.md` now live at `specs/conformance/npm-distribution/`** — the eight slice conformance scripts (`slice-1.sh` through `slice-8.sh`) and four inline variants are committed alongside the sub-spec they verify. The `## Reference Implementations` citation in `specs/npm-distribution.md` and the comment in `reference/moat-npm-publisher.yml` are updated to the new path. No normative change.
