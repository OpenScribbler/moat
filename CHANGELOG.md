...
- **`specs/npm-distribution.md` revocation is hard at the materialization boundary (normative — MUST)** — a Conforming Client that detects a revoked Content Hash MUST refuse to materialize the package and MUST emit a structured log entry naming the revocation source (lockfile entry vs Registry Manifest entry, as a structured `source` field). There is no operator override mechanism in the protocol; an implementation that exposes any per-hash, per-package, or global bypass is non-conformant. The structured log entry is what gives an operator the artifact needed to escalate to the upstream revocation source rather than locally suppressing the refusal.
