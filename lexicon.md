...
| **Lockfile** | The local file a conforming client maintains recording installed content hashes, attestation bundles, signed payloads, registry fetch timestamps, and revoked hashes. The trust anchor for offline verification. | "lock", "install snapshot file" |
...
| **Source URI** | The canonical URL of the source repository for a content item, recorded as `content[].source_uri` in the manifest. | "source URL" (use URI), "origin URL", "repo link" |
