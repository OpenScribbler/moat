...
This is a Draft specification. It has not yet been validated by multiple independent implementations. Before advancing beyond Draft status, test vectors (Appendix B) MUST be confirmed by at least two independent implementations in different languages.
...
The ecosystem has mature standards for content formats and packaging, but no standard addresses the provenance question — verifying who made content, whether it has been tampered with, and where it came from. MOAT fills this gap.

### 2.1 Who Implements MOAT

MOAT is a **platform-side and tooling-side specification**. The entities that implement it are:
...
Individual content creators interact with MOAT through the platforms and tools they use, not by managing cryptographic operations directly. A content author clicks "Publish" in a web interface; the platform computes hashes, signs, and produces the `meta.yaml`. This is the correct layering — MOAT is the interoperability contract between platforms, analogous to TLS for web traffic.

Content without MOAT metadata — unsigned, unverified, shared informally — is a legitimate and expected use case, not a failure state. Permissive consumers (Section 6.2.3) surface the absence of provenance without blocking the user.

### 2.2 The Trust Chain

MOAT provides a four-link trust chain. Each link depends on the previous. Breaking any one invalidates verification.
...
MOAT addresses the trust chain with a single provenance model applied uniformly to all content types. The specification defines:
...
MOAT is designed to be tool-agnostic. Any content management or distribution system can produce and consume MOAT metadata.
...
**publisher** — An entity that computes hashes, optionally signs, and distributes content with MOAT metadata.

**consumer** — An entity that reads, verifies, and acts on MOAT metadata when installing or auditing content.
...
## 5. The meta.yaml Format

### 5.1 File Placement

MOAT metadata is stored in a file named `meta.yaml`, placed in the content directory alongside the content it describes. The filename MUST be exactly `meta.yaml` for all content types.
...
#### 5.3.1 meta_version (REQUIRED)

Integer. The version of the MOAT schema. Currently `1`.
...
> **Note (Informative):** MOAT uses integer versioning to record content snapshots. Semantic versioning (SemVer) is not used because MOAT is a provenance specification, not a package management specification. Breaking-change semantics and dependency resolution are concerns for registries and package managers built on top of MOAT.

#### 5.3.5 description (OPTIONAL)

String. A short description of what the content does. Default: absent.

#### 5.3.6 authors (REQUIRED for team and public scope)
...
#### 5.3.8 source_repo (REQUIRED for public scope)
...
> **Note (Informative):** `source_repo` identifies the repository containing this content. The `derived_from.source` field (Section 5.3.14) may optionally append a path to identify a specific content item within a repository.

#### 5.3.9 source_commit (OPTIONAL, RECOMMENDED for public scope)
...
#### 5.3.10 published_at (REQUIRED for public scope)
...
#### 5.3.15 publisher_identity (CONDITIONAL)
...
The field value is a human-readable identifier for the publishing platform (e.g., `claude.ai`, `acme-platform`).
...
- MUST produce `meta.yaml` files conforming to the schema in Section 5.
...
A conforming registry is any system that indexes or distributes content with MOAT metadata. A conforming registry:
...
Both publishers and consumers MUST use the Directory Tree algorithm (Section 7.3) to compute `content_hash`. All provenance-tracked content resides in a content directory alongside its `meta.yaml` sidecar (Section 5.1), so both parties always operate on a directory — even when the directory contains a single content file.
...
```
MOAT-V1:<content_hash_value>\n<meta_hash_value>
```

Where:

- `MOAT-V1:` is the literal ASCII string (8 bytes: 0x4D 0x4F 0x41 0x54 0x2D 0x56 0x31 0x3A).
...
MOAT does not provide a custom exclusion mechanism (e.g., `.moatignore`) beyond the hardcoded VCS directory exclusion. This is consistent with supply chain integrity tools (Sigstore DSSE, in-toto) that deliberately hash what exists rather than filtering during integrity computation. Exclusion logic in the integrity layer creates an implicit trust boundary — if the exclusion parser has a bug or is crafted maliciously, files silently leave the integrity envelope. Publishers are responsible for curating content directories before publishing.
...
- **Name squatting.** MOAT does not define a publisher namespacing model. Two publishers can publish different content with the same `name`, `version`, and `type`. This is analogous to name squatting in npm and PyPI. Registries SHOULD implement namespacing (Section 5.3.3) to prevent collisions.

- **Version rollback.** Old versions with valid hashes and signatures can be redistributed. MOAT has no freshness mechanism, no "latest version" authority, and no way to mark old versions as superseded. Consumers SHOULD check registries for the latest version rather than relying solely on the `version` field in `meta.yaml`.
...
- **Case-insensitive filesystems.** On macOS (APFS default) and Windows (NTFS), `meta.yaml` and `META.YAML` are the same file. The spec requires exactly `meta.yaml` (Section 5.1), but case-insensitive systems cannot enforce this at the filesystem level. Implementations on case-insensitive systems SHOULD perform case-sensitive filename matching to detect non-conforming filenames.

- **Unicode homoglyph paths.** Visually identical filenames using different Unicode codepoints (e.g., Latin `l` vs Roman numeral `ℓ` U+2113) produce different file paths and different content hashes. NFC normalization (Section 7.3 step 3) does not collapse homoglyphs. Both files hash and verify normally. Homoglyph detection is outside the scope of MOAT.
...
Signing input string (151 bytes):

```
MOAT-V1:sha256:9c9c3591140eae4e0f047060470af98da00629b668f152ac6d4846e64ff91d40\nsha256:ff9c3684845115f9d204b1cef0a8a1b43d7648663ccdea503331bf5623d097d9
```

> **Note:** `\n` represents a single newline byte (0x0A), not the literal characters `\` and `n`. There is no trailing newline.

Signing input hex (151 bytes):

```
4d4f41542d56313a7368613235363a396339633335393131343065616534653066
...
```
sha256:79b10750bbc89db1596266a01fe809112a59e7d7e8fffb7edbaabcf200abd5cf
```

Validates: Section 9.1 — domain separator (`MOAT-V1:`), full `sha256:`-prefixed hash values, single newline separator, no trailing newline. The SHA-256 of the signing input allows implementations to verify correct construction without performing actual signing.

### TV-MH4: Meta hash with publisher_identity and repository_owner_id (Section 8)

Input: Metadata for platform-published content including `publisher_identity` and `repository_owner_id` — the fields added in v0.2.0 for delegated publishing and account resurrection protection. A platform (`acme-platform`) publishes Alice's code-review skill from a platform-managed monorepo.
...
authors:
  - alice <alice@example.com>
source_repo: github.com/acme-platform/community-skills
published_at: "2026-04-01T18:00:00Z"
content_hash: "sha256:9c9c3591140eae4e0f047060470af98da00629b668f152ac6d4846e64ff91d40"
publisher_identity: acme-platform
...
```
{"authors":["alice <alice@example.com>"],"content_hash":"sha256:9c9c3591140eae4e0f047060470af98da00629b668f152ac6d4846e64ff91d40","description":"Code review skill published by a platform on behalf of Alice.","meta_version":1,"name":"community-code-review","published_at":"2026-04-01T18:00:00Z","publisher_identity":"acme-platform","repository_owner_id":"12345678","source_repo":"github.com/acme-platform/community-skills","type":"skill","version":1}
```

```
meta_hash: sha256:9f63655d8c059c40b81b8f5d0a710246ae521f57393794e1b6eece067280e76e
```
...
| `1.3.6.1.4.1.57264.1.17` | Source Repository Owner Identifier | Numeric owner ID (for `repository_owner_id`, Section 5.3.16) |
...
4. Advertise the TUF root via the `sigstore_trust_root` field (Section 5.3.17) in registry manifests.
