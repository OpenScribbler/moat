...
Content without MOAT metadata — unsigned, unverified, shared informally — is a legitimate and expected use case, not a failure state. Permissive consumers (Section 5.2.3) surface the absence of provenance without blocking the user.
...
MOAT is designed to be tool-agnostic. While syllago is the reference implementation, any content management system can produce and consume MOAT metadata.
...
- **Name squatting.** MOAT does not define a publisher namespacing model. Two publishers can publish different content with the same `name`, `version`, and `type`. This is analogous to name squatting in npm and PyPI. Registries SHOULD implement namespacing (Section 6.3.3) to prevent collisions.
