...
- A GitHub repository to host the registry (can be the same repo as your content — see [Self-Publishing Guide](self-publishing.md))
...
The `manifest_uri` should point to the raw file in your repository's default branch — this is where conforming clients and `moat-verify` will fetch the manifest from.

### 2. Copy the workflow file

Copy [`reference/moat-registry.yml`](../../reference/moat-registry.yml) to `.github/workflows/moat-registry.yml` in your registry repo.
...
A successful run commits `registry.json` and `registry.json.sigstore` to your default branch and ends with:
